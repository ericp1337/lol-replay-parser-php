...
class champion_mapper_db
{
    ...
    public function load($champion_id = null)
    {
        ...
        $champion_info = $db->query_first_pdo($champion_query, [$this->get_id()]);
    ...
    {
        $db = db::obtain();
        $db->update_pdo(db::real_tablename('champions'), $champion->get_data_array(), ['id' => $champion->get('id')]);
    }
    ...
    public static function champion_exists($id)
    {
        $id = intval($id);
        $db = db::obtain();
        $r = $db->query_first_pdo('SELECT id FROM '.db::real_tablename('champions').' WHERE id = ?', [$id]);
