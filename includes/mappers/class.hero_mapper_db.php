...
class hero_mapper_db
{
    ...
    public function load($hero_id = null)
    {
        ...
        $db = db::obtain();
        $res = $db->query_first_pdo('SELECT * FROM '.db::real_tablename('champions').' WHERE `key` = ?', [$hero_id]);
        $hero = new hero();
        $hero->set_array($res);
    ...
    {
        $db = db::obtain();
        $db->update_pdo(db::real_tablename('champions'), $hero->get_data_array(), ['key' => $hero->get('key')]);
    }
    ...
    public function delete($hero_id)
    {
        ...
        $db->delete_pdo(db::real_tablename('champions'), ['key' => $id]);
    }
    ...
    public static function hero_exists($hero_id)
    {
        $db = db::obtain();
        $id = intval($hero_id);
        $r = $db->query_first_pdo('SELECT `key` FROM '.db::real_tablename('champions').' WHERE `key` = ?', [$id]);
