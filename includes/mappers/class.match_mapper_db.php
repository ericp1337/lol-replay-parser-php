...
class match_mapper_db extends match_mapper
{
    ...
    public function delete($match_id)
    {
        ...
        $db = db::obtain();
        $slots = $db->fetch_array_pdo('SELECT id FROM '.db::real_tablename('slots').' WHERE gameId = ?', [$match_id]);
        $slots_formatted = [];
        foreach ($slots as $slot) {
        ...
        $db->delete_pdo(db::real_tablename('bans'), ['gameId' => $match_id], 0);
        $db->delete_pdo(db::real_tablename('matches'), ['gameId' => $match_id]);
    }
    ...
    public static function match_exists($match_id = null)
    {
        ...
        $match_id = intval($match_id);
        $db = db::obtain();
        $r = $db->query_first_pdo('SELECT gameId FROM '.db::real_tablename('matches').' WHERE gameId = ?', [$match_id]);
