...
class db
{
    ...
    public static function obtain($server = null, $user = null, $pass = null, $database = null, $pref = null)
    {
        if (is_null(self::$_instance)) {
            self::$_instance = new self($server, $user, $pass, $database, $pref);
        }
    ...
    {
        if (!is_null(self::$_link_id)) {
            return true;
        }
        try {
            self::$_link_id = new PDO('mysql:host='.$this->_server.';dbname='.$this->_database.'', $this->_user, $this->_pass, [PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES \'UTF8\'']);
     ...
     */
    private function _query_pdo($sql, $params = [])
    {
        try {
     ...
     */
    public function query_first_pdo($sql, $params = [])
    {
     ...
     */
    public function fetch_array_pdo($sql, $params = [])
    {
        $query_id = $this->_query_pdo($sql, $params);
        if ($query_id  === false) {
            return false;
     ...
     * @return bool Returns TRUE on success or FALSE on failure.
     */
    public function update_pdo($table, $data, $where = [])
    {
        if (empty($data)) {
            return false;
        }
...
; else {
     ...
     * @return bool
     */
    public function insert_many_pdo($table, array $fields, array $data = [])
    {
        if (empty($data)) {
            return false;
        }
     ...
     */
    public function insert_pdo($table, $data = [])
    {
        if (empty($data)) {
            return false;
        }
     ...
     */
    public function delete_pdo($table, array $where = [], $limit = 1)
    {
        $limit = intval($limit);
        if (empty($where)) {
            return false;
     ...
     * @param mixed $value
     *
     * @return int Returns the corresponding PDO ID of the data type.
     */
